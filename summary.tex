...
\section{Summary of ``Recent Improvements to the McErlang Model Checker''\cite{Earle2009Recent}} 
  \label{section:improve_mcerlang}
\subsection{summary} 
The checking process in McErang is as follows. A
monitor runs in a lock step manner with the program. Between the transition
of two states, the monitor is given the chance to inspect the state of the
whole program, e.g. process mailboxes, process status, node names, process
dictionaries, etc. (All of thses can be viewed as the observable global state
of the erlang program provided by the McErlang runtime system. The type of
information in such global state is very rigid because these information is only
related to a few special system operations like spawn or passing message.) However
it's very difficult to get access to the local state of each process,
e.g. the value of a process variable. The probe annotation provides a way
for users to export the local state information into the monitor. When
McErlang executes the probe statement, it records the information to the
path of actions (if it is a probe of action), or it modifies the global
state provided by McErlang (if it is a probe of state). (Such global state can
be viewed as an extension of the global state mentioned above.) Also users have to
provide corresponding functions to allow the monitor to check the occurrence
of these probes. In the end, users have to provide the monitor to check wheter
the execution of the program doesn't break any requirement, specified based
on those probes. McErlang also provides a tool to automatically generate
monitors from LTF formulae.


In my understanding, McErang is like an interpreter, writtin in Erlang as
well, which can run the erlang program in a special way and connect the
running erlang program with the monitor.
...
\section{Summary of ``Formal Analysis of a Space Craft Controller using SPIN''\cite{Havelund2001Formal}} 
  \label{section:spacecraft_spin}
\subsection{summary}

\subsection{note}
One event can be waited for by multiple tasks. One task can wait for multiple
events. However one task can only require lock for one property, which seems
quite unrealistic.

To better reduce the size of the generated model, high level abstraction
(abstraction of functionality must be provided). The abstraction of
``closure'' in the paper is of this kind. It is difficult to formally argue
that such abstraction is sound or complete for the verification of certain
properties. To argue that the breaking of the property leads to termination of
the task, we make it possible for the task to run forever, tehn check the
property. In the way, we can argue that it's the breaking which causes the
termination, not hte spontaneous behavior of the task.

If we want to check a property like all tasks terminate, then such abstraction
is not good at all.

Misture of code and model is useful.

Currently, it's impossible to reason about everything within the system.
...
\section{Summary of ``Compositional Verification for Component-based Systems and 
  Application''\cite{Bensalem2008Compositional}} 
  \label{section:xxxxxx}
\subsection{summary}
I learned the following concepts from the paper:
1. Component is like a 
2. Invarance of a component
3. What is interaction invariance
...
\section{Summary of ``A Synergy Between Model-Checking and Type Inference for the
Verification of Value-Passing Higher-Order Processes''\cite{Debbabi1999Synergy}} 
  \label{section:xxxxxx}
\subsection{summary}
I haven't finished reading the whole paper. Based on what I understand so far,
this paper creates a programming language based on concurrent ML, a ML like
language equiped with dynamic spawning of new processes and creation of channels.
Both process and channel can be passed via channels like other primitive
constructs. Due to such property of the channel, the language is called a
Value-Passing Higher-Order Processes. The authors give out a type system for such
language. The type of an expression contains the description of the effects caused by
the evaluation of the expression, e.g. creating a channel, receiving a message,
and etc. The authors then give out the concrete operational semantics of a
concurrent program in the flavour of transition system. I didn't quite get the
idea of the abstract operational semantics of programs given by the authors. In
the end, the authors give out the logic that allows one to specify properties of
expressions and the corresponding model-checking algorithm.
...
\section{Summary of ``Temporal Higher-Order Contracts''\cite{Disney2011Temporal}} 
  \label{section:HOC}
\subsection{summary}
xx
...
\section{Summary of ``Java's Insecure Parallelism''\cite{Hansen1999Javas}} 
  \label{section:java_insecure}
\subsection{summary}
The author lists two points for the weakness of Java's synchronized class, based on which 
he also argues that \emph{Java doesn't support a monitor concept}:
\begin{itemize}
\item Unless they are declared as synchronized, Jva class methods are unsynchronized.
\item Unless they are declared as private, Java class variables are public (within a
  package)
\end{itemize}
...
\section{Summary of ``Frenetic: A High-Level Language for OpenFlow Networks''\cite{Foster2010Frenetic}} 
  \label{section:frenetic_openflow}
\subsection{summary}

This paper is relatively easy to read and serves as a good start point for 
studying the status of application of functional reactive programming in the
realm of networking (OpenFlow).


\subsection{quote}
This section presents three examples that highlight the dif-
ficulties of writing programs for the OpenFlow/NOX plat-
form. We have elided a few details of the platform which are
not needed to understand the examples (a complete descrip-
tion is available in the OpenFlow specification [12]).

Although OpenFlow/NOX enables the management of net-
works using arbitrary general-purpose programs, its two-
tiered architecture forces programmers to specify the asyn-
chronous and event-driven interaction between the programs
running on the controller and the switches in the network. In
our experience, these details are a significant distraction and
 a frequent source of bugs.

Frenetic’s unified architecture makes it possible to express
this program as a simple, declarative query.

The core of the run-time system is the back-end, which
manages the installation and uninstallation of rules as well
as all communications between the switches and controller.
...
\section{Summary of ``Proving Liveness Properties of Concurrent Programs''\cite{xxxxxxx}} 
  \label{section:xxxxxx}
\subsection{summary}
xx

\subsection{quote}
In order to specify a concurrent program, one must state which actions are atomic.
Atomic actions are indivisible and represent the finest grain of process
interleaving.

\vspace{5mm} %5mm vertical space

To this end, we describe the semantics of the programming langauge somewhat
informally, but carefully enough so that there should be no ambiguity about the
meaning of programs.

\vspace{5mm} %5mm vertical space

In any implementation, the relative speeds of the processors and the scheduling
mechanism would further constrain the possible execution sequences. However, as long
as all execution sequences are a subset of those in $\Sigma$, any results proved by the
methods in this paper will be true for that implementation.

\vspace{5mm} %5mm vertical space

Temporal logic provides us with both a language for stating program requirements and
a set of rules for reasoning about them.

\vspace{5mm} %5mm vertical space

Our exposition here is brief, and we refer the reader to the above papers for more
detials.
